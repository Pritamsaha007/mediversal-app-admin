...
import { usePathologyTestStore } from "./store/PathologyTestStore";
...
const PathologyTests: React.FC = () => {
  ...
  const { pathologyTests, setPathologyTests } = usePathologyTestStore();
  ...
  const generateStats = (): PathologyStats => {
    const activeTests = pathologyTests.filter((t) => !t.is_deleted);
  ...
  useEffect(() => {
    ...
    const fetchPathologyTests = async () => {
      ...
      try {
        ...
        setPathologyTests(res.labTests);
  ...
  useEffect(() => {
    let filtered = pathologyTests.filter((test) => !test.is_deleted);
    ...
    setCurrentPage(0);
  }, [searchTerm, selectedStatus, pathologyTests]);
  ...
  const handleAddTest = async (newTestData: PathologyTest) => {
    setPathologyTests([...pathologyTests, newTestData]);
    setFilteredTests([
      ...pathologyTests.filter((test) => !test.is_deleted),
      newTestData,
    ]);
  };

  const handleUpdateTest = async (updatedTest: PathologyTest) => {
    const updatedTests = pathologyTests.map((test) =>
      test.id === updatedTest.id ? updatedTest : test,
    );
    setPathologyTests(updatedTests);
  ...
  const handleExport = () => {
    ...
    const testsToExport =
      selectedTests.length > 0
        ? pathologyTests.filter((t) => selectedTests.includes(t.id))
        : filteredTests;
